#ifndef cbi
#define cbi(sfr, bit) (_SFR_BYTE(sfr) &= ~_BV(bit))  // Clear Bit in Register Macro
#endif
#ifndef sbi
#define sbi(sfr, bit) (_SFR_BYTE(sfr) |= _BV(bit))   // Set Bit in Register Macro
#endif
...
unsigned int lineThickness = 26;  // Enter line thickness in mm. Works best for thickness between 10 & 35
...
void loop() {
  ...
  while (digitalRead(11)) {} // Wait until button 11 is pressed
  delay(1000);
  calibrate();  // Calibrate sensors
  while (digitalRead(12)) {} // Wait until button 12 is pressed
  delay(1000);

  while (1) {
    readLine();  // Read line sensors
    if (currentSpeed < lfSpeed) currentSpeed++;  // Increment speed
    if (onLine == 1) {  // If line is detected, execute line following
      linefollow();
...
void linefollow() {
  ...
  motor1run(lsp); // Run left motor
  motor2run(rsp); // Run right motor
}
